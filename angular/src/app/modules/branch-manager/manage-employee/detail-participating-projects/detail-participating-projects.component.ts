...
export class DetailParticipatingProjectsComponent extends AppComponentBase implements OnInit {
  ...
  getData(userId, fromDate, toDate){
    ...
    .subscribe(res => {
      ...
      // if (this.showInactiveProject) {
      // }else{
      //   this.projectList = res.result.getAllValueOfUserInProjectByUserIdDtos.filter(project => project.status === 0);
      // }
      ...
      // });
      // this.totalProjectHour = convertMinuteToHour(sum);
      this.totalWorkingHours = convertMinuteToHour(res.result.totalWorkingHours);
      // this.percentProject = convertHourtoMinute(this.totalWorkingHours) > 0 ? Math.round(convertHourtoMinute(this.totalWorkingHours) * 100 / convertHourtoMinute(this.totalProjectHour)) : 0;
      // this.projectList = res.result.getAllValueOfUserInProjectByUserIdDtos.filter((i) => i.status == 0)
    })
  }
