...
export class TimesheetComponent extends AppComponentBase implements OnInit {
  ...
  onSelectedTypeOfWorkChange() {
    this.filteredTimesheets = this.rawData.filter(
      (s) =>
        this.selectedTypeOfWork === this.APP_CONSTANT.EnumTypeOfWork.All ||
  ...
  buildDataByDate(data: Array<TimeSheetDto>): TimesheetGroupByDayDto[] {
    let typeOfWorkNormalworkinghours =
      this.APP_CONSTANT.EnumTypeOfWork.Normalworkinghours;
    return _.chain(data)
      .groupBy("dateAt")
      .map(
        (value, key) =>
          ...
          } as TimesheetGroupByDayDto)
      )
      .orderBy((x) => x.dateAt, "desc")
      .value();
  }
  warningByDate(value: Array<TimeSheetDto>) {
    let typeOfWorkNormalworkinghours =
      this.APP_CONSTANT.EnumTypeOfWork.Normalworkinghours;
    let totalWorkingTime = value
      .filter((s) => s.typeOfWork === typeOfWorkNormalworkinghours)
      .reduce((total, item) => {
        return (total += item.workingTime + item.offHour * 60);
      }, 0);
    ...
    if (
      value.find(
        (s) => s.typeOfWork === typeOfWorkNormalworkinghours && s.isOffDay
      )
    ) {
    ...
    if (
      isOpenTalkWarning ||
      isLogTsWorkNormalOnOffDay ||
      totalWorkingTime > this.defaultWorkingHourPerDay ||
      (moment(value[0].dateAt, "YYYY-MM-DD").toDate().getDay() ==
        this.APP_CONSTANT.EnumDayOfWeekByGetDay.Saturday &&
    ...
    return _.chain(data)
      ...
      .map(function (value, key) {
        ...
        return {
          ...
          dayTasks: dayTasks,
          isThanDefaultWorkingHourPerDay:
            dayTasks.find(
              (item) => item.isThanDefaultWorkingHourPerDay == true
            ) != undefined
              ? true
              : false,
    ...
    return _.chain(data)
      ...
      .map(function (value, key) {
        ...
        return {
          ...
          checkStatus: that.APP_CONSTANT.CHECK_STATUS.CHECKED_NONE,
          totalProjectWorkingTime: userTasks
            .map((s) => s.totalUserWorkingTime)
            .reduce((a, b) => a + b, 0),
          selectedCount: 0,
  ...
  afterChildChangeWhatStatusParentAre(parent: any, childList: any[]) {
    ...
    childList.forEach((item) => {
      if (item.checkStatus === this.APP_CONSTANT.CHECK_STATUS.CHECKED_ALL)
        countAll++;
      else if (item.checkStatus === this.APP_CONSTANT.CHECK_STATUS.CHECKED_NONE)
        countNone++;
    });
    if (countNone === childList.length)
      parent.checkStatus = this.APP_CONSTANT.CHECK_STATUS.CHECKED_NONE;
    else if (countAll === childList.length)
      parent.checkStatus = this.APP_CONSTANT.CHECK_STATUS.CHECKED_ALL;
  ...
  updateCheckedCount() {
    this.checkedCount = this.rawData.filter(
      (ts) => ts.checked && ts.isUserInProject
    ).length;
  }
  ...
  onProjectCheckedChange(
    $e: MatCheckboxChange,
    project: TimesheetGroupByProjectDto
  ) {
    let isCheckedAll = $e.source.checked;
    project.checkStatus = isCheckedAll
      ? this.APP_CONSTANT.CHECK_STATUS.CHECKED_ALL
      : this.APP_CONSTANT.CHECK_STATUS.CHECKED_NONE;
    project.userTasks.forEach((ut) => {
      ...
      ut.dayTasks.forEach((dt) => {
        ...
        dt.timesheets.forEach(
          (t) => (t.checked = t.isUserInProject ? isCheckedAll : false)
        );
      });
    });
  ...
  onUserCheckedChange(
    $e: MatCheckboxChange,
    user: TimesheetGroupByUserDto,
    project?: TimesheetGroupByProjectDto
  ) {
    //Toggle status
    let isCheckedAll = $e.source.checked;
    user.checkStatus = isCheckedAll
      ? this.APP_CONSTANT.CHECK_STATUS.CHECKED_ALL
      : this.APP_CONSTANT.CHECK_STATUS.CHECKED_NONE;
  ...
  onDayTaskCheckedChange(
    $e: MatCheckboxChange,
    dayTask: TimesheetGroupByDayDto,
    user: TimesheetGroupByUserDto,
    project?: TimesheetGroupByProjectDto
  ) {
    //Toggle status
    let isCheckedAll = $e.source.checked;
    dayTask.checkStatus = isCheckedAll
      ? this.APP_CONSTANT.CHECK_STATUS.CHECKED_ALL
      : this.APP_CONSTANT.CHECK_STATUS.CHECKED_NONE;
    dayTask.timesheets.forEach(
      (t) => (t.checked = t.isUserInProject ? isCheckedAll : false)
    );
  ...
  onTimesheetCheckedChange(
    ...
    if (countNone === dayTask.timesheets.length)
      dayTask.checkStatus = this.APP_CONSTANT.CHECK_STATUS.CHECKED_NONE;
    else if (countAll === dayTask.timesheets.length)
      dayTask.checkStatus = this.APP_CONSTANT.CHECK_STATUS.CHECKED_ALL;
  ...
  btnApprove() {
    ...
    this.rawData.forEach((item) => {
      if (
        item.checked &&
        item.isUserInProject &&
        item.status != this.APP_CONSTANT.TimesheetStatus.Draft
      )
        selectedTimesheets.push(item);
    });
    let approveTimesheetIds = selectedTimesheets
      .filter(
        (x) =>
          x.status != this.APP_CONSTANT.TimesheetStatus.Approve &&
          x.status != this.APP_CONSTANT.TimesheetStatus.Draft
      )
      .map((s) => s.id);

    let timesheetNormalWorkingIds = selectedTimesheets
      .filter(
        (x) =>
          x.status != this.APP_CONSTANT.TimesheetStatus.Approve &&
          x.status != this.APP_CONSTANT.TimesheetStatus.Draft &&
          x.typeOfWork == this.APP_CONSTANT.EnumTypeOfWork.Normalworkinghours
      )
      .map((s) => s.id);
    ...
    var msg =
      approveTimesheetIds.length == 1
        ? `${approveTimesheetIds.length} Timesheet`
        : `${approveTimesheetIds.length} Timesheets`;
    ...
    this.timesheetService
      .getTimesheetWarning(timesheetNormalWorkingIds)
      .subscribe((res: any) => {
        if (res.result.length > 0) {
          this.listTimeSheetWarningDto = res.result;
          this.showTimesheetWarning(
            this.listTimeSheetWarningDto,
            approveTimesheetIds
          );
          return;
        }
  ...
  doApprove(msg, approveTimesheetIds) {
    abp.message.confirm(`Approve ${msg}`, (result: boolean) => {
      if (result) {
        this.timesheetService
          .approveTimesheet(approveTimesheetIds)
          .subscribe((res: any) => {
            if (res) {
              let successMessage =
                res.result.successCount > 0
                  ? `<font color='#029720'>${res.result.success} <br /> </font>`
                  : "";
              let failMessage =
                res.result.failedCount > 0
                  ? `<font color='#e70d0d'>${res.result.fail} <br /> </font>`
                  : "";
              abp.message.info(
                successMessage + failMessage + res.result.lockDate,
                "APPROVED",
                true
              );
              this.getTimesheets();
  ...
  btnReject() {
    let selectedTimesheets = [];
    this.rawData.forEach((item) => {
      if (item.checked) selectedTimesheets.push(item);
    });
    let rejectTimesheetIds = selectedTimesheets
      .filter((x) => x.status != this.APP_CONSTANT.TimesheetStatus.Reject)
      .map((s) => s.id);
    ...
    var msg =
      rejectTimesheetIds.length == 1
        ? `${rejectTimesheetIds.length} Timesheet`
        : `${rejectTimesheetIds.length} Timesheets`;

    abp.message.confirm(`Reject ${msg}`, (result: boolean) => {
      if (result) {
        this.timesheetService
          .rejectTimesheet(rejectTimesheetIds)
          .subscribe((res: any) => {
            if (res) {
              this.getTimesheets();
              this.notify.info(
  ...
  btnExport(type): void {
    var fileName =
      "Invoice_TimeSheets_" + this.fromDate + "_" + this.toDate + "_";
    let selectedTimesheets: TimeSheetDto[] = [];
    this.rawData.forEach((item) => {
      if (item.checked) selectedTimesheets.push(item);
    });
    if (selectedTimesheets.length == 0) {
      abp.message.error("You have to select at least one timesheet");
      return;
    }
  ...
  convertNumberOfStringStatus(status: number, typeOfWork: number) {
    let html =
      typeOfWork != 1
        ? ""
        : '<span class="label label-danger" style="padding: 3px 5px; margin-left: 2npx">OT</span>';
    switch (status) {
      case 0: {
        html =
          "<span style='padding: 3px 5px;font-size: 12px;border-radius: 10px;color: #fff;font-weight: 600;' class='label-warning'>Draft</span>" +
          html;
        break;
      }
      case 1: {
        html =
          "<span style='padding: 3px 5px;font-size: 12px;border-radius: 10px;color: #fff;font-weight: 600;' class='label-info'>Pending</span>" +
          html;
        break;
      }
      case 2: {
        html =
          "<span style='padding: 3px 5px;font-size: 12px;border-radius: 10px;color: #fff;font-weight: 600;' class='label-success'>Approved</span>" +
          html;
        break;
      }
      case 3: {
        html =
          "<span style='padding: 3px 5px;font-size: 12px;border-radius: 10px;color: #fff;font-weight: 600;' class='label-default'>Rejected</span>" +
          html;
        break;
      }
    }
  ...
  dialogSelectDay(type: number) {
    ...
    type == 1
      ? (this.titleExport = "Export Timesheet")
      : (this.titleExport = "Export Remote Request");
  ...
  getAlltTimeSheetOrRemote() {
    ...
    this.timesheetService
      .getAllTimeSheetOrRemote(date, this.exportType)
      .subscribe((res: any) => {
        if (res.result == null) {
