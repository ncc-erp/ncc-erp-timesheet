...
namespace Timesheet.Application.Tests.API.RequestDays
{
    ...
    public class RequestDayAppServiceTest : RequestDayAppServiceTestBase
    {
        ...
        private readonly W2Service _w2Service;

        public RequestDayAppServiceTest() : base()
        {
            _requestDayAppService = InstanceRequestDayAppService();
            this._w2Service = base._w2service;
        }
        ...
        [Fact]
        public async void ApproveRequest_WhenRequestIsRemote_AndRequestInW2IsNotApprovedOrNotFound()
        {

            var requestId = 101;
            var requestDay = AbsenceDayRequest();
            var requestDayDetail = AbsenceDayDetail();
            var dateAt = new DateTime(2023, 09, 07);
            var user = new User();
            user.EmailAddress = "someuser@gmail.com";
            requestDayDetail.DateAt = dateAt;
            requestDay.Status = RequestStatus.Pending;
            requestDay.User = user;
            requestDay.Type = RequestType.Remote;

            // mock data
            WorkFromHomeRequestDto wfhInfo = new WorkFromHomeRequestDto()
            {
                Date = "2023-09-07",
                Email = "someuser@gmail.com",
                Status = WfhW2RequestStatus.NotFound
            };
            _w2Service.GetWfhRequest("someuser@gmail.com", "2023-09-07").Returns(wfhInfo);

            await WithUnitOfWorkAsync(async () =>
            {
                await workScope.InsertAsync(requestDay);
                await workScope.InsertAsync(requestDayDetail);
        ...
        [Fact]
        public async void ApproveRequest_WhenRequestIsRemote_AndRequestInW2IsApproved()
        {
            var requestId = 101;
            var requestDay = AbsenceDayRequest();
            var requestDayDetail = AbsenceDayDetail();
            ...
            WorkFromHomeRequestDto wfhInfo = new WorkFromHomeRequestDto()
            {
                Date = "2023-09-07",
                Email = "someuser@gmail.com",
                Status = WfhW2RequestStatus.Approved
            };
            _w2Service.GetWfhRequest("someuser@gmail.com", "2023-09-07").Returns(wfhInfo);
        ...
        { 
            ...
            var expectedCount = 1;
            var expectedDayOffName = "Nghỉ thông thường\t";
            var expectedFullName = "email6 test";
