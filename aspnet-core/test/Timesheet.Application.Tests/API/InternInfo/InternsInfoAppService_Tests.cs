    ...
    public class InternsInfoAppService_Test : TimesheetApplicationTestBase
    {
        ...
        public async Task Should_Get_All_Interns_Infor_By_SearchText()
        {
            await WithUnitOfWorkAsync(async () =>
            {
                ...
                // Assert
                Assert.NotNull(result);
                result.ListInternInfo.Items.ShouldContain(item => item.MyInfo.EmailAddress.Contains("email12"));
            });
        }
        ...
        public async Task Should_Get_All_Basic_Trainer()
        {
            await WithUnitOfWorkAsync(async () =>
            {
                ...
                result.ShouldContain(item => item.FullName == "email3 test");
                result.ShouldContain(item => item.EmailAddress == "testemail3@gmail.com");
                result.ShouldContain(item => item.Id == 4);
                result.ShouldContain(item => item.FullName == "email4 test");
                result.ShouldContain(item => item.EmailAddress == "testemail4@gmail.com");
            });
        }
