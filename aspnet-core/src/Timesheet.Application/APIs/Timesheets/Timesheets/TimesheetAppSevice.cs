...
namespace Timesheet.Timesheets.Timesheets
{
    ...
    public class TimesheetAppService : AppServiceBase
    {
        ...
        public async Task<List<MyTimeSheetDto>> GetAll(int? opentalkTime, bool? opentalkTimeType, DateTime? startDate, DateTime? endDate, TimesheetStatus status, long? projectId, HaveCheckInFilter? checkInFilter, long? branchId = null, string searchText = "", RequestType? workLocation = null, TypeOfWork? typeOfWork = null, bool? isCharged = null)
        {
            ...
            if (typeOfWork.HasValue)
            {
                queryable = queryable.Where(ts => ts.TypeOfWork == typeOfWork.Value);
            }

            if (isCharged.HasValue)
            {
                queryable = queryable.Where(ts => ts.IsCharged == isCharged.Value);
            }
        ...
        public async Task<object> GetQuantiyTimesheetStatus(int? opentalkTime, bool? opentalkTimeType, DateTime? startDate, DateTime? endDate, long? projectId, HaveCheckInFilter? checkInFilter, string searchText, long? branchId = 0, RequestType? workLocation = null, TypeOfWork? typeOfWork = null, bool? isCharged = null)
        {
            var OpenTalkID = Convert.ToInt64(await SettingManager.GetSettingValueAsync(AppSettingNames.ProjectTaskId));
            var projectIds = await WorkScope.GetAll<ProjectUser>()
                .Where(s => s.UserId == AbpSession.UserId.Value && s.Type == ProjectUserType.PM)
            ...
            if (typeOfWork.HasValue)
            {
                listMyTimesheet = listMyTimesheet.Where(s => s.TypeOfWork == typeOfWork.Value).ToList();
            }
            if (isCharged.HasValue)
            {
                listMyTimesheet = listMyTimesheet.Where(s => s.IsCharged == isCharged.Value).ToList();
            }
