    ...
    public class UserTimeReport_BotAppService : ApplicationService
    {
        ...
        public BotCommandEnvelopeDto ProcessBotCommand([FromBody] BotCommandRequestDto request)
        {
            try
            {
                ...
                string command = SettingManager.GetSettingValueForApplication(AppSettingNames.BotReportCommand);

                if (command.StartsWith("*timesheet", StringComparison.OrdinalIgnoreCase))
                {
                    return ProcessTimesheetCommand( request.ReportDate);
                }
        ...
        public BotCommandEnvelopeDto ProcessTimesheetCommand( DateTime? reportDate)
        {
            try
            {
                string command = SettingManager.GetSettingValueForApplication(AppSettingNames.BotReportCommand);
